...
describe('listFormatter', () => {
  beforeEach(() => {
    ...
    x.transactions = [
      { creator: null, debtor: x.u1, creditor: x.u2, amount: 3, description: 'test-1' },
      { creator: null, debtor: x.u2, creditor: x.u1, amount: 5, description: 'test-2' },
      { creator: x.u1, debtor: x.u1, creditor: x.u2, amount: 7, description: 'test-3' },
      { creator: x.u2, debtor: x.u2, creditor: x.u1, amount: 1, description: 'test-4' },
  ...
  it('should inform if no transactions exist', () => {
    const responseObj = listFormatter(x.u2, [], 0);
    console.log(JSON.stringify(responseObj));
  });
});
